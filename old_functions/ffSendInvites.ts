...
function createActiveLocations(locations) {
    ...
    return locations.map(location => ({
        ...
        older_users: location.groupType === 'older' ? true : false,  
        younger_users: location.groupType === 'younger' ? true : false  
    }));
}
...
async function writeToFirestore(invites, activeLocations) {
   ...
   activeLocations.forEach(activeLocation => {
    ...
    batch.set(activeLocationRef, {
        ...
        older_users: activeLocation.older_users, 
        younger_users: activeLocation.younger_users  
    });
});
