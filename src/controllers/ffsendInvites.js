...
exports.ffsendInvites = functions.https.onRequest(async (request, response) => {
  try {
    console.log("Starting ffsendInvites function");

    const {olderUsers, youngerUsers} = await fetchAndSortUsers();
...
async function processUserGroup(locations, users, groupType) {
  ...
  for (const user of users) {
    const location = findSuitableLocation(locations, user, capacityMultiplier);
    if (!location) {
...
function findSuitableLocation(locations, user, capacityMultiplier) {
  let location;
  if (safeLength(user.preferred_side) > 0) {
    location = locations.find((loc) =>
      user.preferred_side.includes(loc.side_of_town) &&
            loc.usersLength < loc.capacity * capacityMultiplier
    );
  }
  if (!location) {
...
async function writeToFirestore(invites, activeLocations) {
  ...
  invites.forEach((invite) => {
    const inviteRef = admin.firestore().collection("send_sms_invites_tempbin_testing").doc();
    batch.set(inviteRef, invite);
  });
  activeLocations.forEach((activeLocation) => {
    const activeLocationRef = admin.firestore().collection("active_locations_testing").doc(activeLocation.locationId);
